...
# Función BFS para un grafo de enteros
def bfs(grafo, inicio):
    visitados = set()
    cola = [(inicio, 0)]  # Agregar el nivel del nodo al recorrido
    while cola:
        vertice, nivel = cola.pop(0)
        if vertice not in visitados:
            visitados.add(vertice)
            print("Nodo: {} (nivel {})".format(vertice, nivel))  # Imprimir el nivel del nodo
            for adyacente in grafo.get(vertice, []):
                cola.append((adyacente, nivel+1))  # Agregar el nivel del adyacente al recorrido
    return visitados
...
#Funcion que grafica el grafo y las distancias al Nodo inicial
...
# Impresion de BFS, Distancia y grafico con la libreria Matplotlib
print("\nRecorrido BFS:")
bfs(grafo, inicio)
